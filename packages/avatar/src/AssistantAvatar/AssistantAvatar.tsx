...
export const AssistantAvatar: React.ComponentType<AssistantAvatarProps> =
  createGlyphComponent(
    ...
    ({
      ...
      const shimmerId = useIdAllocator({
        prefix: 'lg-assistant_avatar-shimmer',
      });
      const defaultFill = disabled
        ? getDisabledFill(theme)
        : `url(#${gradientId})`;
      const shimmerFill = `url(#${shimmerId})`;
      ...
      return (
        <svg
          ...
          {...rest}
        >
          {/* Gradient definitions used when AssistantAvatar is not in disabled state */}
          {!disabled && (
            <defs>
              <linearGradient
                id={gradientId}
