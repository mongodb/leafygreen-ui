...
| `renderMode`                   | `'inline'` \| `'portal'` \| `'top-layer'` | Options to render the popover element. `'inline'` and `'portal'` are deprecated. `'top-layer'` will render the popover element in the top layer                                                                                                                         | `'top-layer'` |
| `portalContainer` (deprecated) | `HTMLElement`, `null`                     | Sets the container used for the popover's portal.                                                                                                                                                                                                                       |               |
| `scrollContainer` (deprecated) | `HTMLElement`, `null`                     | If the popover portal has a scrollable ancestor other than the window, this prop allows passing a reference to that element to allow the portal to position properly.                                                                                                   |               |
...
| `variant`                      | `'default'`, `'compact'`                  | Determines the visual style and sizing of the menu. Note: When set to `'compact'`, `MenuItem` descriptions will be hidden.                                                                                                                                              | `'default'`   |
...
| `description` | `node`                                             | Content to appear below main text of the `<MenuItem />`. Note: Descriptions are hidden when the parent `<Menu />` has `variant="compact"` |             |
