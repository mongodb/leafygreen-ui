...
- 94745fb: Adds semantically correct `type="button"` to MenuItem components that are rendered as a button. This prevents submitting forms when Menu components are rendered inside of forms.
- Updated dependencies [394ba93]
  - @leafygreen-ui/lib@15.6.0

## 32.1.0

### Minor Changes

- 3c2464c: - Adds `onOpen` and `onClose` callback props to the Menu. These are fired when the popover enter/exit transition (respectively) is completed.
...
- c568585: Improves MenuProps types to extend `React.ComponentProps<'ul'>`
...
- 30b13adec: - Re-enables native event propagation on trigger clicks. Previously this was done to prevent errors caused by different event handling in React vs Backbone.
