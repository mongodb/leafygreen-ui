...
type Result<T, E = unknown> = { value: T } | { error: E };
...
export function within<
  ...
  // source of truth for the handle's type. Type error(s) will occur on the
...
>(
  ...
  const handle = context[enter]();

  if (isPromiseLike(handle)) {
    return castReturn(handle.then(performFn));
  }
  ...
  function performFn(handle: HandleFromEnter<TEnter>) {
    ...
    if ('value' in result && isPromiseLike(result.value)) {
      return Promise.resolve(result.value).catch(async error => {
        await exitContext(handle);
        throw error;
      });
    } else {
      const exitContextPromise = exitContext(handle);

      if (exitContextPromise) {
        return exitContextPromise.then(() => handleResult(result));
      }
