...
It also exports `Theme` and `ThemeType` that allow you to utilize `darkMode` as a enum to more elegantly style components using `cx`.
...
const styles: Record<ThemeType, string> = {
  [Theme.Light]: css`
    /* styles here */
  `,
  [Theme.Dark]: css`
