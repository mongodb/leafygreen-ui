...
<>{createUniqueClassName('ComponentName')}</>

#### Why does this exist?

Emotion no longer supports using the return value from `css` within other styles, which limits the ability to use things like well-scoped sibling and child selectors. The original use of selectors generated outside of emotion to workaround this limitation was actually given to us by the maintainers of Emotion themselves, as they know this is a limitation of the library.
