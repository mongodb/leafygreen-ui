 ...
 * @checksum 9ef6fe326705325bf2e8d213c3602dd1
 */
...
const idAllocator = IdAllocator.create('ArrowLeft');
...
const ArrowLeft = ({
  ...
  const fillStyle = css`
    color: ${fill};
  `;
  title = getGlyphTitle('ArrowLeft', title);
  return (
    <svg
      className={cx(
        {
    ...
    >
      {title === undefined ? (
        <title id={titleId}>{'Glyphs / Arrow / Left'}</title>
      ) : title ? (
