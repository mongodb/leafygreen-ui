...
interface BaseOverlineProps {
  className?: string;
}

type OverlineProps<C extends React.ElementType> = OverrideComponentProps<
  C,
  BaseOverlineProps
>;

function Overline<C extends React.ElementType = 'div'>({
  className,
  ...rest
}: OverlineProps<C>) {
  return (
    <Box
      component="div"
      className={cx(sharedStyles, overline, className)}
      {...rest}
    />
  );
}
