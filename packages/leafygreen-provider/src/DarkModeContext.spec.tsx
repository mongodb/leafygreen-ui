...
describe('packages/leafygreen-provider/DarkModeContext', () => {
  ...
  test('without a provider, by default colorScheme returns "light"', () => {
    const { container } = render(<TestComponent />);
    expect(container.innerHTML.includes('colorScheme: light')).toBeTruthy();
  });
  ...
  test('with a provider, by default colorScheme returns "light"', () => {
    const { container } = render(
      <LeafyGreenProvider>
        <TestComponent />
      </LeafyGreenProvider>,
  ...
  test(`when darkMode prop is set to true, colorScheme returns dark`, () => {
    const { container } = render(
      <LeafyGreenProvider darkMode={true}>
        <TestComponent />
      </LeafyGreenProvider>,
    ...
    expect(container.innerHTML.includes('colorScheme: dark')).toBeTruthy();
  });
});
