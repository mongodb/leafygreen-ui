...
const TestComponent = () => {
  const { checkedState, isChecked, isSelectAllChecked, isSelectAllIndeterminate, toggleSeries, toggleSelectAll } = useSeriesContext();

  return (
    <div>
      <div data-testid="checkedState">{Array.from(checkedState).join(',')}</div>
      <div data-testid="isChecked">{isChecked('test-id').toString()}</div>
      <div data-testid="isSelectAllChecked">{isSelectAllChecked().toString()}</div>
      <div data-testid="isSelectAllIndeterminate">{isSelectAllIndeterminate().toString()}</div>
      <button data-testid="toggleSeries" onClick={() => toggleSeries('test-id')}>Toggle Series</button>
      <button data-testid="toggleSelectAll" onClick={toggleSelectAll}>Toggle Select All</button>
    </div>
  );
};
...
const renderTestComponentWithProvider = (series: Array<string>) => {
  ...
  const isSelectAllIndeterminateEl = utils.getByTestId('isSelectAllIndeterminate');
