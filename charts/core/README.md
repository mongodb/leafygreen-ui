...
| `customRow` _(optional)_            | Renders an additional row (e.g. a total) at the bottom of the tooltip's series list. Receives the hovered series' names and values. Return `null` to hide the row. Not affected by `sort` and not shown in the `Legend`. | `(seriesData: Array<CustomRowSeriesInfo>) => CustomRowInfo \| null` |                         |
