...
| `headerFormatter` _(optional)_      | Callback function for formatting the header string.                                                                                                                                                                      | `(value: number \| string, index: number) => string \| ReactNode`                |                         |
| `customRow` _(optional)_            | Renders an additional row (e.g. a total) at the bottom of the tooltip's series list. Receives the hovered series' names and values. Return `null` to hide the row. Not affected by `sort` and not shown in the `Legend`. | `(seriesData: Array<CustomRowSeriesInfo>) => CustomRowInfo \| null \| undefined` |                         |

Note: `SeriesInfo` is of type `{ name: string; value: string | number | Date; }`, `CustomRowSeriesInfo` is of type `{ name: string; value: string | number | Date | null | undefined; }` (value is nullish when a series has no data point at the hovered axis value), and `CustomRowInfo` is of type `{ name: ReactNode; value: ReactNode; }`
