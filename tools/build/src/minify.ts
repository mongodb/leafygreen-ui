...
export function minifyPackage({ verbose, glob: globPatterns }: MinifyOptions) {
  ...
  for (const jsFile of jsFiles) {
    ...
    const minifiedFile = path.join(dir, `${baseName}-min.js`);
    const minifiedFileMap = path.join(dir, `${baseName}-min.js.map`);
    const existingSourceMapFile = path.join(dir, `${baseName}.js.map`);
