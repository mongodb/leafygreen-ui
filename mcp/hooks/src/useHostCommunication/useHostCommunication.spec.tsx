...
jest.mock('@mcp-ui/server', () => ({
    ...
    uiActionResultIntent: jest.fn((intent, data) => ({ type: 'intent', intent, data })),
    uiActionResultNotification: jest.fn((message) => ({ type: 'notification', message })),
    uiActionResultPrompt: jest.fn((prompt) => ({ type: 'prompt', prompt })),
    uiActionResultToolCall: jest.fn((tool, args) => ({ type: 'tool', tool, args })),
...
describe('mcp/hooks/useHostCommunication', () => {
    ...
    test('intent() creates and posts intent action', () => {
        ...
        expect(uiActionResultIntent).toHaveBeenCalledWith('create-task', { title: 'Test' });
        expect(postUIActionResult).toHaveBeenCalledWith({ type: 'intent', intent: 'create-task', data: { title: 'Test' } });
    });
    ...
    test('notify() creates and posts notification action', () => {
        ...
        expect(postUIActionResult).toHaveBeenCalledWith({ type: 'notification', message: 'Task created' });
    });
    ...
    test('prompt() creates and posts prompt action', () => {
        ...
        expect(postUIActionResult).toHaveBeenCalledWith({ type: 'prompt', prompt: 'What is your name?' });
    });
    ...
    test('tool() creates and posts tool call action', () => {
        ...
        expect(uiActionResultToolCall).toHaveBeenCalledWith('search', { query: 'test' });
        expect(postUIActionResult).toHaveBeenCalledWith({ type: 'tool', tool: 'search', args: { query: 'test' } });
    });
    ...
    test('link() creates and posts link action', () => {
        ...
        expect(postUIActionResult).toHaveBeenCalledWith({ type: 'link', url: 'https://example.com' });
    });
